# Network
  ...
  - [4. 네트워크 계층-데이터 평면](#4-네트워크-계층-데이터-평면)
    ...
    - NAT
  - [5. 네트워크 계층-제어 평면](#5-네트워크-계층-제어-평면)
    - 포워딩 테이블은 어떻게 만들어지는가
    - 라우팅 알고리즘
    - OSPF
    - BGP
...
하지만, 이미 네트워크 장비는 너무나 많고, 사용자의 수에 따라 ISP에서 사용자들에게 IP 주소를 할당하기 위한 서브넷의 주소 범위가 변해야 할 수도 있다.  
<br>
NAT는 이런 문제를 해결해준다. NAT는 간단하게 사설 IP를 할당해주는 기술인데, NAT 가능 라우터의 운영을 그림으로 살펴보자.  
<br>
<img width="1117" alt="image" src="https://user-images.githubusercontent.com/76645095/179455201-9850f903-609c-40ea-bd05-18e8f26cfcb0.png">
<br>
NAT 가능 라우터는 그림의 오른쪽 처럼 홈 네트워크의 일부인 인터페이스를 가진다. 그림에서는 홈 네트워크의 4개의 인터페이스 모두 같은 네트워크 주소 10.0.0.0/24를 가지는 상황이다.  

이 주소 공간은 '사설망'으로 네트워크 주소들이 그 네트워크의 내부에 있는 장비에게만 의미가 있는 네트워크이다.  
따라서 이 사설망 내의 홈 네트워크 장비는 10.0.0.0/24 주소체계를 이용하여 서로 패킷을 송수신할 수있다. 그러나 홈 네트워크를 벗어나 인터넷으로 가는 패킷 전달은 이 주소들을 사용할 수없다.  
<br>
왜냐하면, 10.0.0.0/24 주소를 사용하는 수십만개의 네트워크가 있기 때문이다. 즉, 10.0.0.0/24 주소들은 홈 네트워크 내부에서만 의미가 있다.  
그렇다면, 이 사설망에서 인터넷과 송수신을 하고싶다면 어떻게 해야할까?? 여기서 NAT가 사용된다.  

'NAT 가능' 라우터는 외부 세계에서 볼때 라우터처럼 보이지 않고, 하나의 IP 주소를 가지는 장비로 동작한다.  
NAT 가능 라우터는 사성망 내의 호스트들이 보내는 패킷을 '대신' 전달하는 역할을 하면서 내부 호스트들을 숨기는 기능을 수행한다.  
사설망 Host의 패킷은 모두 NAT 가능 라우터를 거쳐야 하고, NAT 가능 라우터는 이 패킷의 src IP 주소를 자신의 인터페이스 IP 주소로 변경한다.  
<br>
따라서 외부에서 IP 데이터 그램을 열어보면, Src IP 주소가 NAT 가능 라우터며 이 라우터가 패킷을 보낸것 처럼 '착각'한다.  
그러면, NAT 라우터는 자신의 IP를 어떻게 얻고, Host는 어디서 주소를 얻는가?에 대한 의문이 생긴다. 그리고 여기서 DHCP 가 사용된다.  
<br>
라우터는 ISP의 DHCP 서버로부터 주소를 얻고, NAT로 제어되는 홈 네트워크의 주소 공간에서 DHCP 서버를 실행하여 host 컴퓨터에게 주소를 제공한다.  
<br>
그러면, 결국 서로 다른 호스트가 패킷을 외부 네트워크로 보낼때 response의 Dest IP 주소는 모두 NAT 라우터일 것이다.  
여기서 라우터는 어떻게? 적절한 사설망 내 호스트에게 이 패킷을 보낼 수있을까?  
<br>
이를 가능하게 도와주는 기법이 NAT에서 'NAT 변환 테이블'을 이용하는 것이다. 이 테이블에는 IP 주소와 포트 번호를 매핑하고 있다.  
<br>
<img width="1127" alt="image" src="https://user-images.githubusercontent.com/76645095/179458456-e004eaa6-b644-4a9a-8c91-15e572391bc8.png">

<br>
이 그림을 예시로 사용하면, 10.0.0.1:3345의 host를 138.76.29.7:5001에 매핑하여 NAT 변환 테이블에 저장하고 있고, 이 정보를 사용하면 response의 Dest에 따라 적절한 host로 다시 전달할 수있는 것이다.  

NAT 테이블에서 WAN 측에 저장되는 정보는, IP 주소는 전부 같고 port 번호는 전부 다르게 저장된다.   
NAT 가능 라우터는 자신이 사용하지 않는 어떠한 포트 번호나 LAN 측의 Host와 매핑하는 방식으로 테이블을 관리한다. 


## 5. 네트워크 계층-제어 평면
### 포워딩 테이블은 어떻게 만들어지는가
포워딩 테이블은 네트워크 계층의 데이터 평면과 제어 평면을 연결하는 주요 요소인데, 이 테이블들이 어떻게 만들어지고, 유지되고, 설치되는지 먼저 알아보자.  

크게 2가지 방법이 존재한다.  
1. 라우터별 제어  
<img width="1560" alt="image" src="https://user-images.githubusercontent.com/76645095/179502778-62077347-221b-42be-a54a-bdde911999b7.png">
<br>
위의 그림은 라우팅 알고리즘들이 모든 라우터 각각에서 동작하는 경우를 보여준다.   
포워딩과 라우팅 기능이 모두 개별 라우터에 포함되어 있으며, 각 라우터는 다른 라우터의 라우팅 구성요소와 통신하여 자신의 포워딩 테이블의 값을 계산하는 라우팅 구성요소를 가지고 있다.  

2. 논리적으로 중앙 집중된 제어  
<img width="1680" alt="image" src="https://user-images.githubusercontent.com/76645095/179503678-d4805f75-c261-4ff8-8197-802750a3f412.png">  
<br>
위의 그림은 논리적으로 집중된 컨트롤러가 포워딩 테이블을 작성하고, 이를 모든 개별 라우터가 사용할 수있도록 배포한 경우를 나타낸다.  
컨트롤러는 잘 정의된 프로토콜을 통해 각 라우터의 '제어 에이전트(CA)'와 상호 작용하여 라우터의 플로우 테이블을 구성 및 관리한다.  
<br>
일반적으로 CA는 컨트롤러와 통신하고, 컨트롤러의 명령을 수행하는 최소한의 기능만을 가진다.  
CA는 서로 상호 작용하지 않으며 포워딩 테이블을 계산하는 데에도 적극적으로 참여하지 않는다.  

### 라우팅 알고리즘
라우팅 알고리즘의 목표는 송신자부터 수신자까지 라우터의 네트워크를 통과하는 좋은 경로를 결정하는 것이다. 여기서 '좋은 경로'란 일반적으로 '최소 비용 경로'를 의미한다.  

네트워크 제어 평면이 라우터별 제어 방식을 선택하든, 중앙 집중된 방식을 채택하든 상관없이 패킷이 이동하기 위해서는 일련의 라우터가 항상 존재해야 한다.  
이는 라우터의 경로를 계산하는 라우팅 알고리즘은 근본적으로 중요한 이유이다.  

라우팅 문제를 나타내는 데에는 그래프가 사용되며, 라우팅 알고리즘을 분류하는 일반적인 방법 한 가지는 알고리즘이 중앙 집중형인지, 분산형인지이다.  

1. 중앙 집중형 라우팅 알고리즘 (링크 상태 알고리즘)   
네트워크 전체에 대한 완전한 정보를 가지고 출발지와 목적지 사이의 최소 비용 경로를 계산한다.  
즉, 이 알고리즘은 모든 노드 사이의 연결 상태와 링크 비용을 입력값으로 사용하며 이러한 정보는 알고리즘이 계산을 수행하기 전 얻어야 한다.  
<br>
계산 자체는 하나의 중앙 집중된 컨드롤러에서 수행하거나, 모든 라우터 각각의 라우팅 모듈로 복사될 수있다.  
<br>
이 알고리즘의 가장 큰 특성은 '연결'과 '링크 비용'에 대한 완전한 정보를 가지다는 점이다. 이렇게 전체 상태 정보를 가지고 있는 알고리즘을 '링크 상태(link-state)' 알고리즘이라고 부른다.  
<br>

2. 분산 라우팅 알고리즘  (거리 벡터 알고리즘)  
최소 비용 경로의 계산이 라우터들에 의해 반복적이고 분산된 방식으로 수행된다.  
<br>
어떤 노드도 모든 링크의 비용에 대한 완전한 정보를 가지고 있지 않으며, 각 노드는 자신에 직접 연결된 링크에 대한 비용 정보만을 가지고 시작한다.  
이후 반복된 계산과 이웃 노드와의 정보 교환을 통해 노드는 점차적으로 목적지 또는 목적지 집합까지의 최소 비용 경로를 계산한다.  
이를 '거리 벡터' 알고리즘이라고 부른다.

### OSPF
인터넷은 ISP들의 네트이크이고, 각 ISP들은 자신의 라우터들로 구성된 네트워크이다.  
이렇게 ISP로 네트워크를 분할하는 이유는, 라우터의 수가 증가함에 모든 라우터 사이의 연결 상태와 링크 비용을 브로드캐스팅 하기 위해 필요한 오버헤드가 엄청나기 때문이다.  
<br>
이 문제를 해결하기 위해 라우터들을 '자율 시스템(autonomous system, AS)'으로 조직화할 수있는데, 각 AS는 동일한 관리 제어하에 있는 라우터의 그룹으로 구성된다.  
각 AS는 전세계적으로 고유한 AS 번호로 식별되며, 같은 AS 안에 있는 라우터들은 동일한 라우팅 알고리즘을 사용하며 상대방에 대한 정보를 가지고 있다.  
<br>
OSPF는 링크 상태 정보를 플러딩하고, 다익스트라 최소 비용 경로 알고리즘을 사용하는 '링크 상태 알고리즘'이다. OSPF를 이용하여 각 라우터는 전체 AS에 대한 완벽한 그래프를 얻고, 자신을 루트 노드로 두고 모든 서브넷에 이르는 최단 경로 트리를 경정하기 위해 다익스트라 알고리즘을 수행한다.
<br>
<br>
OSPF를 사용하는 라우터는 인전합 라우터만이 아니라 자율 시스템 내의 모든 라우터에게 라우팅 정보를 브로드캐스트한다. 또한 링크 상태가 변경되면, 변경될 때마다 링크 상태 정보를 브로드캐스트하고, 변경이 생기지 않더라도 주기적으로 링크 상태를 브로드캐스트한다.  
이러한 링크 상태의 주기적인 갱신은 링크 상태 알고리즘에 견고성을 더해준다.  

### BGP
동일한 AS 내에 있는 출발지와 목적지 사이에서 패킷을 라우팅할 때, 패킷이 전송되는 경로는 전적으로 AS 내부 라우팅 프로토콜에 의해 결정된다.  
그러나, 패킷이 여러 AS를 통과하도록 라우팅할 때 '자율 시스템 간 라우팅 프로토콜'이 필요하다.  
AS간 라우팅 프로토콜은 여러 AS 간의 협력이 수반되므로, 통신하는 AS들은 같은 AS간 라우팅 프로토콜을 사용해야 한다. 그리고 인터넷의 모든 AS들은 'BGP(Border Gateway Protocol)'이라고 불리는 동일한 AS간 라우팅 프로토콜을 사용한다.  
<br>
AS간 라우팅 프로토콜로서 BGP는 각 라우터에게 아래와 같은 수단을 제공한다.  
1. 이웃 AS로부터 도달 가능한 서브넷 접두부 정보를 얻는다.
2. 서브넷 주소 접두부로의 가장 좋은 경로를 결정한다.

### ICMP
ICMP(Internet Control Message Protocol)은 호스트와 라우터가 서로 간에 네트워크 계층 정보를 주고받기 위해 사용되며, 가장 전형적인 사용 형태는 '오류 보고'이다.  
라우터는 호스트에게 네트워크 상에서 오류가 발생했음을 알리기 위해 ICMP 메시지를 만들어서 보내며, 이 메시지는 IP 페이로드로 전달된다.  
<br>
중요한 것은 ICMP가 무조건적으로 오류 상태를 알리기 위해서 사용되는 것은 아니라는 것이다.


## 6. 링크 계층
### 링크 계층의 역할
링크 계층의 기본 서비스는 단일 통신 링크상으로 데이터그램을 한 노드에서 인접 노드로 '이동'시키는 것이지만, 서비스의 세부사항은 링크에 적용되는 특정 링크 계층 프로토콜을 따르며 링크 계층 프로토콜이 제공할 수 있는 서비스는 다음과 같은 것들이 존재한다.  
- 프레임화 : 거의 모든 링크 계층 프로토콜은 네트워크 계층 데이터그램을 링크상으로 전송하기 전에 링크 계층 프레임에 캡슐화한다. 
- 링크 접속 : 링크 접속 프로토콜은 링크상으로 프레임을 전송하는 규칙에 대해서 명시한다.
- 신뢰적 전달 : 링크 계층 프로토콜이 신뢰적 전달 서비스를 제공하는 경우 네트워크 계층 데이터그램은 링크 상에서 오류 없이 전달된다. 트랜스포트 계층의 신뢰적 전달 서비스와 마찬가지로 링크 계층의 신뢰적 전달 서비스도 '확인응답'과 '재전송'을 통해 가능해지며, 무선 링크처럼 오류율이 높은 링크에서 주로 사용된다. 하지만, 낮은 비트 오류율을 가진 링크에서는 불필요한 오버헤드가 될 수 있기 때문에 대다수 유선 링크 계층 프로토콜은 신뢰적 전달 서비스를 제공하지 않는다. 
- 오류 검출과 정정 : 신호의 약화나 전자기잡음 때문에, 수신 노드의 링크 계층 하드웨어는 비트를 오인할 수 있다. 오류 검출은 송신 노드에서 프레임에 오류 검출 비트를 설정하게 하고, 수신 노드에서 오류 검사를 수행하게 함으로써 가능해진다. 
  
### 링크 계층이 구현되는 위치
링크 계층은 '네트워크 인터페이스 카드(NIC)'로 알려진 네트워크 어댑터에 구현된다.  
네트워크 어댑터의 중심에는 링크 계층 제어기가 있으며, 링크 계층 제어기는 앞 절에서 설명한 링크 계층 서비스들의 대다수가 구현되어 있는 단일의 특수 용도 칩이다. 

따라서 링크 계층 제어기의 기능 대부분은 '하드웨어'로 구현된다. 
