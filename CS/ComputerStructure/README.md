...
## 5. 보조 저장장치
### 보조 저장장치가 느린 이유
거의 모든 컴퓨터시스템에서 보조저장장치로 가장 널리 사용되는 '하드 디스크(hard disk)'의 원래 명칭은 '자기적 하드 디스크'이며 자화될 수 있는 물질로 코팅된 플라스틱이나, 금속을 이용한 '원형 평판'으로 만들어지는데, 그 평판 위에 '헤드'라고 불리는 전도성 코일을 통해 표면을 자화시킴으로써 데이터를 저장하고 읽는다.  

이 보조저장장치는 주기억장치와 비교했을때 매우 느리다. 하지만, 하드웨어적으로 왜 느린지 좀더 구체적인 공부가 필요해 정리하려고 한다.  

디스크가 느리다는 것은 '액세스 시간'을 말하는 것이다. 아래의 그림을 예시로 보조저장장치의 '액세스 시간'에 대해 알아보자.  

<br>
디스크의 표면에는 여러 동심원들이 존재하는데 아래의 그림과 같다. 

<img width="1018" alt="image" src="https://user-images.githubusercontent.com/76645095/179680619-f9d422af-21eb-48f8-bc6a-a452a7bc33bf.png">
<br>

그림에 나타난 여러 원들을 '트랙(track)'이라고 부르며, 트랙은 또 세부적으로 '섹터'라는 단위로 쪼개진다. 데이터들은 이 '섹터' 위에 저장된다.  
헤드는 트랙을 지나가는 동안 데이터를 저장하거나 인출하게 된다.  
<br>
트랙 상에 데이터를 저장하는 동작을 '디스크 쓰기'라고 부르며, 쓰기 동작은 헤드의 코일에 전류가 흐르면 '자기장'이 발생하는 전기적 성질을 이용한다.  
즉, 전기적 신호가 보내지면 전류의 방향에 따라 서로 다른 형태의 자성 패턴이 트랙 표면에 기록된다.  
또한 저장된 데이터를 디스크 표면으로부터 감지하여 인출하는 동작을 '디스크 읽기'라고 부른다. 읽기 동작은 자기장이 존재하는 곳에 코일이 지나가면 코일에 전기가 유기되는 전기적 성질을 이용한다.  
즉, 디스크 표면이 회전하는 동안에 원하는 데이터가 저장되어 있는 트랙 부분이 헤드 밑을 통과하면 기록할때와 동일한 극성의 전류가 생성된다.  
<br>
이러한 데이터 읽기와 저장은 디스크 표면의 트랙에 액세스해야 가능한 작업들인데, 이는 헤더를 통해서만 가능하다.  

헤더를 이용해 디스크에 저장된 어떤 데이터 블록을 읽거나 쓰는 과정은 아래와 같다.  
1. 헤드를 해당 트랙으로 이동시킨다.  
2. 원하는 데이터가 저장된 섹터가 헤드 아래로 회전되어 올때까지 기다린다.  
3. 데이터를 전송한다.  

위의 3가지 과정에서 걸리는 전체 시간을 '디스크 액세스 시간(Disk Access Time)'이라고 부른다.  
특히 1번에 걸리는 시간을 '탐색 시간'이라고 하며, 2번에 걸리는 시간을 '회전 지연시간'이라고 부른다. 그리고 3번은 '데이터 전송 시간'이라고 부르는데, 여기에는 디스크 제어기에서 소요되는 전자회로의 동작 시간도 포함된다.  

결론적으로 디스크에 저장된 하나의 데이터 블록을 액세스하는 데 걸리는 시간은 다음과 같이 계산할 수있다.  
``` 
디스크 액세스 시간 = 탐색 시간 + 회전 지연 시간 + 데이터 전송 시간
```

먼저 '탐색 시간'을 살펴보자.  
헤더의 이동은 기계적인 이동이 수행되는 것이므로 '시동 시간(startup time)'도 필요하며 여러 트랙들을 지나는데 적어도 수 ms가 걸리고, 거리가 먼 경우에는 그 이상도 걸린다.  

'회전 지연시간'은 디스크의 회전 속도에 달려있다.  
아주 운이 좋게 헤드가 해당 트랙에 도착한 순간 바로 원하는 섹터가 헤드 아래에 즉각 도달한 경우에는 회전 지연시간이 0이 된다.  
그러나 만약 원하는 섹터가 그 직전에 지나갔다면, 헤드가 다시 한 바퀴를 돌아올 때까지 기다려야 한다.  
<br>
따라서 평균 회전 지연 시간은 해당 섹터가 반 바퀴 회전한 후에 헤드 아래에 도달하는 데 걸리는 시간이 된다.  
<br>
마지막 요소인 '데이터 전송 시간'은 헤드가 해당 섹터의 시작 부분에 위치한 순간부터 데이터 비트들을 전송하는 데 걸리는 시간을 의미한다.  
이 시간은 '블록의 크기', '회전 속도', '트랙의 저장 밀도', '버스 전송률', '내부 전자회로의 속도', 디스크 경합으로 인한 '디스크 큐에서의 대기시간' 등이 포함되기에 매우 가변적이다.  

<br>


하드 디스크에 접근하고 사용하기 위해서는 위와같이 수많은 '기계적'인 작업들이 요구된다.

이들 작업은 사람의 입장에서는 매우 빠르게 수행되는 '것'처럼 보인다.  하지만 논리적인 작업과 비교했을때 이 수행 과정은 매우 느린 시간이며 주기억장치에 비해서 접근하는 속도도 매우 느리기 때문에 보조기억장치를 '느리다'라고 말할 수있는 것이다.

### RAID
RAID는 다수의 작은 디스크들을 배열로 연결하여 용량을 늘리고 신뢰성을 향상시킨 대용량 디스크 시스템이다.  

디스크의 저장 밀도가 계속 높아짐에 따라 메가바이트 당 가격이 떨어지고, 시스템이 보유 할 수있는 디스크 용량은 증가하고 있다.  
이와같은 추세에 따라, 하나의 대형 디스크를 사용하는 것보다는 크기가 작은 여러 개의 디스크들을 서로 연결하여 하나의 큰 용량을 가진 디스크 유니트로 사용하는 것이 보다 저렴한 가격으로 더 큰 용량을 가진 디스크 서브시스템을 구성할 수있는 방법이 된다.  
<br>
디스크는 컴퓨터의 프로그램 실행 속도에 직접적으로 영향을 주는 보조저장장치이기 때문에 '액세스 속도'와 '신뢰도 향상'이 용량의 증가만큼 중요하다.  
디스크의 속도를 향상시키기 위해서는 '액세스 시간'을 줄이고 '데이터 전송 속도'를 높여야 하는데 용량을 늘리기 위해 배열 구조로 연결한 다수의 작은 디스크들에 데이터를 분산 저장하게 되면 '동시 액세스'가 가능해져 액세스 시간이 줄어든다.  
<br>
또한 디스크마다 별도의 전송 채널을 가지므로 병렬 전송이 가능해져 '데이터 전송 시간'도 크게 단축시킬 수있다.  
<br>
데이터 블록들을 여러 개의 디스크에 분산 저장하는 기술을 '디스크 인터리빙'이라고 부르는데 어떤 파일이 다수의 데이터 블록들로 이루어져 있을때 디스크 인터리빙 기술을 이용하면, 블록들을 서로 다른 디스크들에 균등하게 분산 저장할 수있다.  

예시로 아래의 그림을 보자.  
<img width="1026" alt="image" src="https://user-images.githubusercontent.com/76645095/179724151-b25e21d3-d094-487a-b580-78718594d001.png">
<br>
4개의 디스크들로 구성된 디스크 배열에 16개의 블록들로 이루어진 파일을 저장한다고 가정하자. 그러면 위의 그림처럼 데이터 블록 B1은 첫 번째 디스크에 저장하고, B2, B3, B4는 순서대로 다음 디스크들에 저장한다.  
<br>
그리고 블록 수가 디스크 수보다 더 많으므로 다섯 번째 블록인 B5는 다시 첫 번째 디스크에 저장하고, 위와 같은 순서를 계속 반복하게 된다.  
결과적으로 데이터 블록들이 4개의 디스크들에 균등하게 분산 저장되어있다.

따라서 각 블록들을 동시에 쓰거나 읽는 동작이 독립적으로 수행될 수 있고, 어느 한 디스크에 액세스가 집중되지 않기 때문에 병목 현상도 줄일 수 있다.

하지만, 어떤 한 디스크에라도 결함이 발생하면 전체 데이터 파일이 손상되는 문제가 발생할 수있다. 

RAID의 장단점을 정리해보면 아래와 같다.  
- 장점 : 데이터 전송률, 동시 입출력 처리율, 가격 등
- 단점 : 장치에 결함이 발생하는 평균 시간 간격이 짧아진다.  

연결된 디스크의 수가 증가함에 따라 신뢰도가 낮아지는 RAID의 단점을 해결하기 위한 한가지 해결법으로 디스크 배열에 '오류 검출 및 복구를 위한 여분의 디스크'를 추가하는 방법이 있다.  


이러한 디스크를 '검사 디스크'라고 부르며, 배열 내의 한 디스크에 결함이 발생하면 그 디스크의 사용을 중단하고 검사 디스크에 저장된 정보를 이용하여 원래의 정보를 신속히 재구성하여 여분의 디스크에 저장한다.  
따라서 RAID를 사용하는 시스템에서 프로그램 실행 중에 어떤 디스크에 결함이 발생하더라도 중단될 필요가 없어졌다. 
